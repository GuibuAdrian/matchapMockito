...
public class PlayerDAO implements PlayerDAOI {
	...
	public Player read(int id) {
		return (Player) playerDAO.playerList.stream().filter(player -> player.getId() == id).findFirst().get();
	}

	public Player read(String name) {
		return (Player) playerDAO.playerList.stream().filter(player -> player.getName().equals(name)).findFirst().get();
	}
